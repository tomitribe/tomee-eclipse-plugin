 ...
 * Tomcat 70 handler.
 */
public class TomEE15Handler implements ITomcatVersionHandler {
	/**
	 * @see ITomcatVersionHandler#verifyInstallPath(IPath)
	 */
	public IStatus verifyInstallPath(IPath installPath) {
		IStatus result = TomcatVersionHelper.checkCatalinaVersion(installPath, TomcatPlugin.TOMEE_15);
		// If check was canceled, use folder check
		if (result.getSeverity() == IStatus.CANCEL) {
			result = TomcatPlugin.verifyInstallPathWithFolderCheck(installPath, TomcatPlugin.TOMEE_15);
		}
		return result;
	}
	...
	public IStatus canAddModule(IModule module) {
		...
		if ("jst.web".equals(module.getModuleType().getId())) {
			...
			if ("1.1".equals(version) || "2.0".equals(version) || "2.1".equals(version) || "3.0".equals(version)
					|| "3.1".equals(version))
				return Status.OK_STATUS;
			
		} else if ("jst.ear".equals(module.getModuleType().getId())) {
			if ("1.4".equals(version) || "5.0".equals(version) || "6.0".equals(version))
				return Status.OK_STATUS;
		}
